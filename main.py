...
def main(config): 
    ...
    model, _ = xclip.load(config.MODEL.PRETRAINED, config.MODEL.ARCH, 
                         ...
                         logger=logger,
                         temporal_tokens=config.MODEL.TEMPORAL_TOKENS,
                        )
    model = model.cuda()
...
def train_one_epoch(epoch, model, criterion, optimizer, lr_scheduler, train_loader, text_labels, config, mixup_fn):
    ...
    temporal_loss_meter = AverageMeter()
    start = time.time()
    end = time.time()
    ...
    for idx, batch_data in enumerate(train_loader):
        ...
        total_loss = criterion(output, label_id) + t1_loss*0.5 + t2_loss*0.3 + t3_loss*0.2
        #total_loss = criterion(output, label_id) 
        total_loss = total_loss / config.TRAIN.ACCUMULATION_STEPS
        temporal_loss = (t1_loss*0.5 + t2_loss*0.3 + t3_loss*0.2)/ config.TRAIN.ACCUMULATION_STEPS
